...
def evaluate_ballot(dict1, remaining = None):
    ...
    for key, value in dict1.items():
        ...
        for i in range(len(value)):
            if not_accessed:
                if value[i] in remaining and value[i] in list(counted_dict.keys()):
                    counted_dict[value[i]] += 1
                    not_accessed = False
                elif value[i] in remaining and value[i] not in list(counted_dict.keys()):
                    counted_dict[value[i]] = 1
                    not_accessed = False
    return counted_dict
...
if __name__== '__main__':
    ...
    remain = set()
    count = 1
    for candidate_lst in candidates:
